...
module Elasticsearch
  module API
    ...
    module Utils
      ...
      #
      # @api private
      private def listify(*list)
        puts "HEY LOOOK: #{list}"
        options = list.last.is_a?(Hash) ? list.pop : {} of String => String
      ...
      #
      # @api private
      private def __pathify(*segments)
        Array.new(1,segments).first.flatten.
          compact.
          reject { |s| s.to_s =~ /^\s*$/ }.
          join('/').
          squeeze('/')
      ...
      # @api private
      #
      private def __validate_and_extract_params(arguments, params=[], options={} of Symbol => String)
      ...
      private def __validate_params(arguments, valid_params=[])
      ...
      private def __extract_params(arguments, params=[], options={} of String => String)
      ...
      # @api private
      #
      private def __extract_parts(arguments, valid_parts=[])
      ...
      private def __report_unsupported_parameters(arguments, params=[])
        messages = []
