module Elasticsearch
  module Transport
    ...
    #
    class Client
      DEFAULT_TRANSPORT_CLASS  = Transport::HTTP::Cossack
      ...
      def initialize(arguments={} of Symbol => Char, &block)
        ...
        @arguments[:transport_options]  ||= {} of Char => Char
        @arguments[:http]               ||= {} of Char => Char

        @arguments[:transport_options].update(:request => { :timeout => @arguments[:request_timeout] } ) if @arguments[:request_timeout]

        @arguments[:transport_options][:headers] ||= {} of Char => Char
        ...
        @transport       = @arguments[:transport] || begin
          if transport_class == Transport::HTTP::Faraday
            #transport_class.new(:hosts => __extract_hosts(hosts, @arguments), :options => @arguments) do |cossack|
            #  block.call cossack if block
            #  unless (h = cossack.builder.handlers.last) && h.name.start_with?("Cossack::Adapter")
            #    cossack.adapter(@arguments[:adapter] || __auto_detect_adapter)
            #  end
            #end
          else
      ...
      # @api private
      #
      def __extract_hosts(hosts_config, options={} of Char => Char)
