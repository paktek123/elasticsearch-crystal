    ...
    module Transport
      module Connections
        ...
        class Connection
          ...
          getter :host, :connection, :options, :failures, :dead_since
          ...
          # @return [String]
          #
          def full_url(path, params={} of Char => Char)
            url  = "#{host[:protocol]}://"
          ...
          # @return [String]
          #
          def full_path(path, params={} of Char => Char)
            path + (params.empty? ? '' : "?#{HTTP::Cossack::ParamsHash[params].to_query}")
          end
